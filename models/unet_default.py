...
def unet_default(model_name, input_dim):
    ...
    # Block 1
    en_b1 = Conv2D_block(input=Input_img, element_num=3, filters=(filter_factor*math.pow(2, 0)), kernel_size=conv2D_kernel_size, padding=padding, kernel_initializer=kernel_initializer, activation=activation, block_name='encoder_b1')
    en_b1_pooling = MaxPooling2D(resample_size, strides=resample_strides, padding=padding, name='block1_pool')(en_b1)

    # Block 2
    en_b2 = Conv2D_block(input=en_b1_pooling, element_num=3, filters=(filter_factor*math.pow(2, 1)), kernel_size=conv2D_kernel_size, padding=padding, kernel_initializer=kernel_initializer, activation=activation, block_name='encoder_b2')
    ...
    # Block 2
    de_b2 = Conv2DTranspose(filters=(filter_factor*math.pow(2, 1)), kernel_size=resample_size, strides=resample_strides, padding=padding, name='decoder_b2_convT')(de_b1)
    de_b2 = Concatenate(axis=3)([en_b2, de_b2])
    de_b2 = Conv2D_block(input=de_b2, element_num=3, filters=(filter_factor*math.pow(2, 1)), kernel_size=conv2D_kernel_size, padding=padding, kernel_initializer=kernel_initializer, activation=activation, block_name='decoder_b2')

    # Block 3
    de_b3 = Conv2DTranspose(filters=(filter_factor*math.pow(2, 0)), kernel_size=resample_size, strides=resample_strides, padding=padding, name='decoder_b3_convT')(de_b2)
    de_b3 = Concatenate(axis=3)([en_b1, de_b3])
    de_b3 = Conv2D_block(input=de_b3, element_num=3, filters=(filter_factor*math.pow(2, 0)), kernel_size=conv2D_kernel_size, padding=padding, kernel_initializer=kernel_initializer, activation=activation, block_name='decoder_b3')
    ...
    # Output
    output = Conv2D(filters=1, kernel_size=(1, 1), padding=padding, kernel_initializer='glorot_uniform', name='output_conv')(decoder)
