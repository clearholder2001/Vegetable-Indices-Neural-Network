...
def Conv2DTrans_block(input, element_num, filters, kernel_size, strides, padding, kernel_initializer, activation, block_name):
    input_layer = input
    for i in range(1, element_num+1):
        layer = Conv2DTranspose(filters=filters, kernel_size=kernel_size, strides=strides, padding=padding, kernel_initializer=kernel_initializer, name="{0}_convT{1}".format(block_name, i))(input_layer)
        layer = BatchNormalization(name="{0}_T_bn{1}".format(block_name, i))(layer)
        layer = Activation(activation, name="{0}_T_ac{1}".format(block_name, i))(layer)
        input_layer = layer
    return layer


def unet_0506(model_name, input_dim):
    ...
    # Block 1
    # de_b1 = Conv2DTrans_block(input=encoder, element_num=1, filters=(filter_factor*math.pow(2, 3)), kernel_size=kernel_size, strides=strides, padding=padding, kernel_initializer=kernel_initializer, activation=activation, block_name='decoder_b1')
    ...
    # Block 2
    de_b2 = Conv2DTrans_block(input=encoder, element_num=1, filters=(filter_factor*math.pow(2, 2)), kernel_size=kernel_size, strides=strides,
                              padding=padding, kernel_initializer=kernel_initializer, activation=activation, block_name='decoder_b2')
    ...
    # Block 3
    de_b3 = Conv2DTrans_block(input=de_b2, element_num=1, filters=(filter_factor*math.pow(2, 1)), kernel_size=kernel_size, strides=strides,
                              padding=padding, kernel_initializer=kernel_initializer, activation=activation, block_name='decoder_b3')
    ...
    # Block 4
    de_b4 = Conv2DTrans_block(input=de_b3, element_num=1, filters=(filter_factor*math.pow(2, 0)), kernel_size=kernel_size, strides=strides,
                              padding=padding, kernel_initializer=kernel_initializer, activation=activation, block_name='decoder_b4')
    ...
    # Output
    output = Conv2D(filters=1, kernel_size=kernel_size, padding=padding, kernel_initializer='glorot_uniform', name='output_conv')(decoder)
